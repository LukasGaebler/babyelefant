...
class autoShape(nn.Module):
    # input-robust model wrapper for passing cv2/np/PIL/torch inputs. Includes
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        # Image.open('image2.jpg'), ...]  # list of images
        ...
        for i, im in enumerate(imgs):
            if isinstance(im, str):  # filename or uri
                im = Image.open(
                    requests.get(
                        im,
                        stream=True).raw if im.startswith('http') else im)  # open
        ...
        shape1 = [
            make_divisible(
                x, int(
                    self.stride.max())) for x in np.stack(
