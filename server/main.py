...
def create_app(test=False):
    ...
    #db.engine.execute(text(open("init.sql").read()))
    db.create_all()

    if test:
        for line in open('data.sql').read().split('\n'):
    ...
    proc = multiprocessing.Process(target=evaluateImagesLoop)
    proc.start()
...
if(__name__) == "__main__":
    ...
    socketio,app = create_app(test=False)
