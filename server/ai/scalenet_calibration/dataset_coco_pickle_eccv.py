...
from ai.scalenet_calibration.utils.keypoint import PersonKeypoints
...
def make_bins_layers_list(x_bins_lowHigh_list):
    ...
    for layer_idx, x_bins_lowHigh in enumerate(x_bins_lowHigh_list):
        ...
        x_bins_centers = np.append(
            x_bins_centers, x_bins_centers[-1])  # 42 bins
        x_bins_layers_list.append(x_bins_centers)
    return x_bins_layers_list

# yc_bins_centers_1 = np.append(yc_bins_centers_1, yc_bins_centers_1[-1])
# # 42 bins


bins_lowHigh_list_dict = {}

# yc_bins_lowHigh_list = [[0.5, 3.], [-0.3, 0.3], [-0.15, 0.15], [-0.15,
# 0.15], [-0.05, 0.05]] # 'SmallerBins'
yc_bins_lowHigh_list = [[0.5, 5.], [-0.3, 0.3], [-0.15,
                                                 0.15], [-0.3, 0.3], [-0.15, 0.15]]  # 'YcLargeBins'
# yc_bins_lowHigh_list = [[0.5, 10.], [-0.5, 0.5], [-0.15, 0.15], [-0.3,
# 0.3], [-0.15, 0.15]] # 'YcLargerBinsV2'
...
fmm_bins_lowHigh_list = [[0., 0.], [-0.2, 0.2], [-0.05,
                                                 0.05], [-0.05, 0.05], [-0.05, 0.05]]  # percentage!!
...
v0_bins_lowHigh_list = [[0., 0.], [-0.15, 0.15], [-0.05,
                                                  0.05], [-0.05, 0.05], [-0.05, 0.05]]  # 'SmallerBins'
...
human_bins_lowHigh_list = [[0., 0.], [-0.3, 0.15],
                           [-0.10, 0.10], [-0.10, 0.10], [-0.05, 0.05]]  # 'SmallerBins'
...
car_bins_lowHigh_list = [[0., 0.], [-0.10, 0.10], [-0.05,
                                                   0.05], [-0.10, 0.10], [-0.05, 0.05]]  # 'SmallerBins'
...
# pickle_paths['train-val'] =
# '/data/COCO/coco_results/results_with_kps_20200221_Car_noSmall-ratio1-35-mergeWith-kps_20200208_morethan2_2-8/pickle'
# #MultiCat
...
bbox_paths = {key: pickle_paths[key].replace(
    '/pickle', '/npy') for key in pickle_paths}
...
class COCO2017Scale(torchvision.datasets.coco.CocoDetection):
    def __init__(
        ...
        assert split in ['train', 'val',
                         'test'], 'wrong dataset split for COCO2017Scale!'
        ...
        if split == 'train':
            self.yannick_mat_files = self.yannick_mat_files[:int(
                num_mat_files * 0.8)]
        elif split == 'val':
            self.yannick_mat_files = self.yannick_mat_files[-int(
                num_mat_files * 0.2):]
        ...
        if self.train:
            self.img_filenames = [os.path.basename(yannick_mat_file).split(
                '.')[0] for yannick_mat_file in self.yannick_mat_files]
            self.img_files = [
                os.path.join(
                    image_path,
                    img_filename +
                    '.jpg') for img_filename in self.img_filenames]
            self.bbox_npy_files = [
                os.path.join(
                    bbox_path,
                    img_filename +
                    '.npy') for img_filename in self.img_filenames]
            self.pickle_files = [
                os.path.join(
                    pickle_path,
                    img_filename +
                    '.data') for img_filename in self.img_filenames]
            assert len(
                self.bbox_npy_files) == len(
                self.pickle_files) == len(
                self.pickle_files) == len(
                self.img_files) == len(
            ...
            for bbox_npy_file, pickle_file, img_file, yannick_mat_file in zip(
                    self.bbox_npy_files, self.pickle_files, self.img_files, self.yannick_mat_files):
                if os.path.isfile(pickle_file):
                    assert os.path.basename(bbox_npy_file)[
                        :12] == os.path.basename(pickle_file)[
                        :12] == os.path.basename(img_file)[
                        :12] == os.path.basename(yannick_mat_file)[
                        :12]
            ...
            self.img_filenames = [os.path.basename(pickle_file).split(
                '.')[0] for pickle_file in self.pickle_files]
            self.img_files = [
                os.path.join(
                    image_path,
                    img_filename +
        ...
        if shuffle:
            ...
            list_zip = list(zip(self.img_files,
                                self.bbox_npy_files,
                                self.pickle_files,
                                self.yannick_mat_files))
            random.shuffle(list_zip)
            self.img_files, self.bbox_npy_files, self.pickle_files, self.yannick_mat_files = zip(
                *list_zip)
            assert os.path.basename(
                self.img_files[0])[
                :12] == os.path.basename(
                self.bbox_npy_files[0])[
                :12] == os.path.basename(
                    self.pickle_files[0])[
                        :12] == os.path.basename(
                            self.yannick_mat_files[0])[
                                :12]
        ...
        if not self.train:
            print([os.path.basename(img_file)
                   for img_file in self.img_files[:12]])
    ...
    def __getitem__(self, k):
        ...
        target_boxes = torch.as_tensor(
            bboxes).reshape(-1, 4)  # guard against no boxes
        target = BoxList(
            target_boxes,
            im_ori_RGB.size,
        ...
        if self.opt.est_kps:
            ...
            classes = [self.json_category_id_to_contiguous_id[c]
                       for c in classes]
        ...
        if self.train and self.opt.est_kps:
            target_maskrcnnTransform.add_field(
                "keypoints_ori", target_keypoints)
            target_maskrcnnTransform.add_field("boxlist_ori", target)
        target_maskrcnnTransform.add_field(
            'img_files', [self.img_files[k]] * num_boxes)
        ...
        if self.train:
            ...
            #     v0_single = yc * (vt - vb) / y_person + vb
                yc_single = y_person * \
                    (v0 - vb) / (vt - vb) / (1. + (vc - v0) * (vc - vt) / f_pixels_yannick**2)
        ...
        return im_yannickTransform, im_maskrcnnTransform, W, H, float(yc_estCam), self.pad_bbox(
            bboxes, self.GOOD_NUM).astype(
            np.float32), bboxes.shape[0], float(v0), float(f_pixels_yannick), os.path.basename(
            self.img_files[k])[
                :12], self.img_files[k], target_maskrcnnTransform, labels
