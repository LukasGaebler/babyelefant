...
def merge_bboxes(bboxes):
    ...
    for bbox in bboxes:
        max_x1y1x2y2 = [min(max_x1y1x2y2[0], bbox[0]), min(max_x1y1x2y2[1], bbox[1]), max(
            max_x1y1x2y2[2], bbox[2] + bbox[0]), max(max_x1y1x2y2[3], bbox[3] + bbox[1])]
    return [
        max_x1y1x2y2[0],
        max_x1y1x2y2[1],
        max_x1y1x2y2[2] -
...
def check_clear(ann, vis=False, debug=False):
    ...
    if debug:
        print(eyes_ys_valid[should_min_y_idx], np.min(kps_valid[:, 1]), kps[15:17, 1]
              [should_max_y_idx], np.max(kps_valid[:, 1]), kps[1:5, 2], kps[15:17, 2])
...
def fmm_to_fpix(f, H, W):
    # full-frame sensor: 43mm (36mm * 24mm)
    sensor_diag = np.sqrt(36**2 + 24**2)
    img_diag = np.sqrt(H**2 + W**2)
