...
class RandomSaturation:
    def __call__(self, sample):
        if np.random.rand() < 0.75:
            ...
            im = sample[0]
            im = torch.clamp(
                im + (torch.max(im, 0)[0] - im) * saturation_amt, 0, 1)
            sample[0] = im
        return sample


perturb = transforms.ColorJitter(
    brightness=0.2,
    contrast=0.2,
    saturation=0.2,
    hue=0.1)
...
def make_data_loader(
    ...
    if is_train:
        ...
        images_per_gpu = images_per_batch // num_gpus if batch_size_override == - \
            1 else batch_size_override
        shuffle = True
