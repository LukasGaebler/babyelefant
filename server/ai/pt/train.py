...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if pretrained:
        ...
        logger.info(
            ...
            (len(state_dict), len(
                model.state_dict()), weights))  # report
    else:
    ...
    for epoch in range(
        ...
        if opt.image_weights:
            ...
            if rank != -1:
                indices = (
                    torch.tensor(
                        dataset.indices) if rank == 0 else torch.zeros(
                        dataset.n)).int()
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    # tb_writer.add_graph(model, imgs)  # add model to
                    # tensorboard
                elif plots and ni == 10 and wandb:
                    wandb.log({"Mosaics": [wandb.Image(str(x), caption=x.name)
    ...
    if rank in [-1, 0]:
        ...
        if plots:
            ...
            if wandb:
                ...
                wandb.log({"Results": [wandb.Image(
                    str(save_dir / f), caption=f) for f in files if (save_dir / f).exists()]})
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for conf, iou, save_json in (
                    [0.25, 0.45, False], [0.001, 0.65, True]):  # speed, mAP tests
...
if __name__ == '__main__':
    ...
    if opt.resume:  # resume an interrupted run
        ckpt = opt.resume if isinstance(
            opt.resume, str) else get_latest_run()  # specified or most recent path
        ...
        opt.data, opt.cfg, opt.hyp = check_file(
            opt.data), check_file(
            opt.cfg), check_file(
            opt.hyp)  # check files
        assert len(
