    ...
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        if os.path.isfile(sources):
            with open(sources, 'r') as f:
                sources = [
                    x.strip() for x in f.read().strip().splitlines() if len(
                        x.strip())]
        else:
            sources = [sources]
...
def img2label_paths(img_paths):
    ...
    return [x.replace(sa, sb, 1).replace(
        '.' + x.split('.')[-1], '.txt') for x in img_paths]
...
def cutout(image, labels):
    ...
    def bbox_ioa(box1, box2):
        ...
        inter_area = (np.minimum(b1_x2,
                                 b2_x2) - np.maximum(b1_x1,
                                                     b2_x1)).clip(0) * (np.minimum(b1_y2,
                                                                                   b2_y2) - np.maximum(b1_y1,
                                                                                                       b2_y1)).clip(0)
