...
def git_describe():
    ...
    if Path('.git').exists():
        return subprocess.check_output(
            'git describe --tags --long --always', shell=True).decode('utf-8')[:-1]
    else:
        return ''
...
def profile(x, ops, n=100, device=None):
    ...
    for m in ops if isinstance(ops, list) else [ops]:
        ...
        p = sum(
            list(
                x.numel() for x in m.parameters())) if isinstance(
            m,
            nn.Module) else 0  # parameters
        print(
...
def find_modules(model, mclass=nn.Conv2d):
    ...
    return [
        i for i, m in enumerate(
            model.module_list) if isinstance(
            m, mclass)]
...
def model_info(model, verbose=False, img_size=640):
    ...
    try:  # FLOPS
        ...
        img = torch.zeros(
            (1, model.yaml.get(
                'ch', 3), stride, stride), device=next(
