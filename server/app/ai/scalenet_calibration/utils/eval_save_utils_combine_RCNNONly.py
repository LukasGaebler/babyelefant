...
def check_eval_COCO(
        ...
        epochs_evaled):
    is_better = False
    # if epoch != 0 and epoch != epoch_start and not opt.not_val and epoch not
    # in epochs_evaled:
    if not opt.not_val and epoch not in epochs_evaled:
        ...
        with torch.no_grad():
            ...
            if isinstance(scheduler, ReduceLROnPlateau) and rank == 0:
                ...
                # step_metrics, epoch, scheduler.num_bad_epochs,
                # scheduler.patience, scheduler.best)))
                    ...
                    writer.add_scalar(
                        ...
                        tid)
                    writer.add_scalar(
                        'training/scheduler-best', scheduler.best, tid)
                    writer.add_scalar(
...
def check_vis_coco(
        ...
        savepath_cocoPose='',
        if_eval_mode=True):
    if not opt.not_val and (
            epoch < opt.save_every_epoch or epoch %
            opt.save_every_epoch == 0) and epoch not in epochs_evaled:
...
def check_save(
    ...
    # if rank == 0 and epoch != 0 and (epoch < opt.save_every_epoch or epoch %
    # opt.save_every_epoch == 0) and epoch not in epochs_saved:
    if rank == 0 and (
            epoch_save < opt.save_every_epoch or epoch_save %
            opt.save_every_epoch == 0) and epoch_save not in epochs_saved:
        ...
        saved_filename = checkpointer.save(
            'checkpointer_epoch%04d_iter%07d' %
            (epoch_total, tid), **arguments)
    ...
    if rank == 0 and is_better:
        ckpt_filepath = 'best_checkpointer_epoch%04d_iter%07d' % (
            epoch_total, tid)
