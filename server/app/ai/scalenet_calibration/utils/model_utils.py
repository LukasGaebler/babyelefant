...
from ai.scalenet_calibration.dataset_coco_pickle_eccv import bins_lowHigh_list_dict, yc_bins_centers, human_bins, yc_bins_layers_list, fmm_bins_layers_list, v0_bins_layers_list, human_bins_layers_list, car_bins, car_bins_layers_list
# from dataset_coco import bins_lowHigh_list_dict
# horizon_bins_centers
from ai.scalenet_calibration.dataset_cvpr import pitch_bins_centers, roll_bins_centers, vfov_bins_centers

from ai.scalenet_calibration.utils.bounding_box import BoxList
...
def approx_model(input_dict):
    yc_est, vb, y_person, v0, vc, f_pixels_yannick = input_dict['yc_est'], input_dict[
        'vb'], input_dict['y_person'], input_dict['v0'], input_dict['vc'], input_dict['f_pixels_yannick']
    inv_f2 = 1. / (f_pixels_yannick * f_pixels_yannick)
    vt_camEst = (yc_est * vb + y_person * (v0 - vb) * (1. + inv_f2 * (vc - v0)
                                                       * vc)) / (yc_est + y_person * (v0 - vb) * inv_f2 * (vc - v0))
    return vt_camEst
...
def accu_model(input_dict, if_debug=False):
    ...
    print('---z2', (f_pixels_yannick * torch.sin(theta_yannick) -
                    (vc - vb) * torch.cos(theta_yannick) + 1e-10))

    if if_debug:
        print('---z', z)
    if z.detach().cpu().numpy() < 0:
        if if_debug:
            print('----z----=====', z, theta_yannick / np.pi * 180.,
                  f_pixels_yannick, yc_est, theta_yannick, vc, vb)
        negative_z = True
...
def accu_model_helanyi(input_dict, if_debug=False):
    ...
    Rt = torch.tensor([[1, 0, 0, 0], [0, cos, sin, -yc * cos],
                       [0, -sin, cos, yc * sin]]).to(device)
...
def get_pitch_est_bad(
    ...
    pitch_bins_low_device_batch_v0 = 0.5 - \
        torch.tan(- pitch_bins_low_device_batch) * \
        f_estim.unsqueeze(-1) / H_batch.unsqueeze(-1)
    pitch_bins_high_device_batch_v0 = 0.5 + \
        torch.tan(pitch_bins_high_device_batch) * \
        f_estim.unsqueeze(-1) / H_batch.unsqueeze(-1)
    ...
    expectation_pitch_bins_low_device_batch_v0 = (torch.exp(nn.functional.log_softmax(
        output_pitch[:, :low_dims], dim=1)) * pitch_bins_low_device_batch_v0).sum(dim=1)
    expectation_pitch_bins_mid_device_batch_v0 = (torch.exp(nn.functional.log_softmax(
        output_pitch[:, low_dims:(low_dims + mid_dims)], dim=1)) * pitch_bins_mid_device_batch).sum(dim=1)
    expectation_pitch_bins_high_device_batch_v0 = (torch.exp(nn.functional.log_softmax(
        output_pitch[:, -high_dims:], dim=1)) * pitch_bins_high_device_batch_v0).sum(dim=1)
    ...
    expectation_batch_v0 = (torch.argmax(output_pitch,
                                         dim=1) < 31).float() * expectation_pitch_bins_low_device_batch_v0 + (torch.argmax(output_pitch,
                                                                                                                           dim=1) >= 224).float() * expectation_pitch_bins_high_device_batch_v0 + ((torch.argmax(output_pitch,
                                                                                                                                                                                                                 dim=1) >= 31).float() * (torch.argmax(output_pitch,
                                                                                                                                                                                                                                                       dim=1) < 224).float()) * expectation_pitch_bins_mid_device_batch_v0
...
def get_pitch_est_v0(bins, output_pitch, H_batch, vfov_estim, f_estim):
    ...
    # half_fov_batch = (vfov_estim / 2.).unsqueeze(-1)
    pitch_bins_low_device_batch_v0 = 0.5 - \
        torch.tan(- bins['pitch_bins_low_device_batch']) * \
        f_estim.unsqueeze(-1) / H_batch.unsqueeze(-1)
    pitch_bins_high_device_batch_v0 = 0.5 + \
        torch.tan(bins['pitch_bins_high_device_batch']) * \
        f_estim.unsqueeze(-1) / H_batch.unsqueeze(-1)
    pitch_bins_mid_device_batch_v0 = bins['pitch_bins_mid_device_batch'].repeat(
        pitch_bins_low_device_batch_v0.shape[0], 1)
    ...
    expectation_batch_v0 = (torch.exp(nn.functional.log_softmax(
        output_pitch, dim=1)) * pitch_bins_device_batch_v0).sum(dim=1)
    return expectation_batch_v0
...
def get_pitch_est_v0_mid(bins, output_pitch, H_batch, vfov_estim, f_estim):
    ...
    expectation_batch_v0 = (torch.exp(nn.functional.log_softmax(
        output_pitch[:, 31:224], dim=1)) * pitch_bins_mid_device_batch_v0).sum(dim=1)
    return expectation_batch_v0
    ...
    expectation_batch_v0 = (
        torch.exp(
            nn.functional.log_softmax(
                output_pitch,
                dim=1)) *
        bins['pitch_bins_v0_wide_device_batch']).sum(
        dim=1)
    return expectation_batch_v0


def get_horizon_est_yannick(bins, output_horizon):
    horizon_bins_device_batch = bins['horizon_bins_centers_torch'].unsqueeze(
        0).repeat(output_horizon.shape[0], 1)
    expectation_batch_horizon = (torch.exp(nn.functional.log_softmax(
        output_horizon, dim=1)) * horizon_bins_device_batch).sum(dim=1)
    # ([Yannick] 0 = top of the image, 1 = bottom of the image)
    return expectation_batch_horizon


def get_pitch_radian_est_yannick(bins, output_pitch):
    pitch_bins_device_batch = bins['pitch_bins_centers_torch'].unsqueeze(
        0).repeat(output_pitch.shape[0], 1)
    expectation_batch_pitch = (torch.exp(nn.functional.log_softmax(
        output_pitch, dim=1)) * pitch_bins_device_batch).sum(dim=1)
    # ([Yannick] 0 = top of the image, 1 = bottom of the image)
    return expectation_batch_pitch
...
def get_bins(device):
    vfov_bins_centers_torch = torch.from_numpy(
        vfov_bins_centers).float().to(device)
    ...
    yc_bins_centers_torch = torch.from_numpy(
        yc_bins_centers).float().to(device)
    human_bins_torch = torch.from_numpy(human_bins).float().to(device)

    pitch_bins_low_device_batch = torch.from_numpy(
        pitch_bins_low).float().to(device).unsqueeze(0)
    pitch_bins_mid_device_batch = (torch.arange(
        0, 193).float().to(device) / 192).unsqueeze(0)
    pitch_bins_high_device_batch = torch.cat((torch.from_numpy(pitch_bins_high).float(
    ).to(device), torch.Tensor([np.pi / 6]).float().to(device))).unsqueeze(0)
    pitch_bins_v0_wide_device_batch = torch.from_numpy(
        pitch_bins_v0_wide).float().to(device).unsqueeze(0)
...
def get_bins_combine(device):
    ...
    vfov_bins_centers_torch = torch.from_numpy(
        vfov_bins_centers).float().to(device)
    roll_bins_centers_torch = torch.from_numpy(
        roll_bins_centers).float().to(device)
    pitch_bins_centers_torch = torch.from_numpy(
        pitch_bins_centers).float().to(device)
    horizon_bins_centers_torch = torch.from_numpy(
        horizon_bins_centers).float().to(device)
    ...
    yc_bins_layers_list_torch = [torch.from_numpy(yc_bins_layer).float().to(
        device) for yc_bins_layer in yc_bins_layers_list]

    fmm_bins_layers_list_torch = [torch.from_numpy(fmm_bins_layer).float().to(
        device) for fmm_bins_layer in fmm_bins_layers_list]

    v0_bins_layers_list_torch = [torch.from_numpy(v0_bins_layer).float().to(
        device) for v0_bins_layer in v0_bins_layers_list]

    human_bins_layers_list_torch = [torch.from_numpy(human_bins_layer).float().to(
        device) for human_bins_layer in human_bins_layers_list]

    car_bins_layers_list_torch = [torch.from_numpy(car_bins_layer).float().to(
        device) for car_bins_layer in car_bins_layers_list]
    ...
    bins_return.update({'car_bins_torch': car_bins_torch,
...
def bboxArray_to_bboxList(
    ...
    for bboxes_array, bboxes_length, W, H in zip(
            bboxes_batch_array, bboxes_length_batch_array, W_batch_array, H_batch_array):
...
def human_prior(tensor, mean=1.70, std=0.103):
    return 1. / np.sqrt(2. * np.pi * (std**2)) * \
        torch.exp(-(tensor - mean)**2 / (2. * std**2))
