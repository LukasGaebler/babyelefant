...
from ai.scalenet_calibration.utils import utils_coco
from ai.scalenet_calibration.dataset_coco_pickle_eccv import bin2midpointpitch
import glob
import numpy as np
...
def save_checkpoint(
    ...
    best_path = os.path.join(os.path.join(
        save_folder, task_name), 'best_' + filename)
    latest_path = os.path.join(os.path.join(
        save_folder, task_name), 'latest.pth.tar')
...
def clean_up_checkpoints(
    ...
    list_checkpoints = list(filter(os.path.isfile, glob.glob(
        checkpoint_folder + '/%s*.*' % start_with)))
    ...
    if len(list_checkpoints) > leave_N:
        for checkpoint_path in list_checkpoints[leave_N:]:
            ...
            if last_saved is not None and ntpath.basename(
                    last_saved) == ntpath.basename(checkpoint_path):
...
def printensor(msg):
    def printer(tensor):
        if tensor.nelement() == 1:
            ...
            print(
                f"min/max/mean: {tensor.max()}, {tensor.min()}, {tensor.mean()}")
    return printer


def summary_thres(array, threses, writer, name, tid):
    assert len(
        array.shape) == 1, 'summary_thres only applies to array of shape [N,]!'
    thres_ratio_dict = {}
...
def load_densenet(model, ckpt_name, checkpoints_folder, opt, old=False):
    ...
    if torch_version >= '1.3.0':
        pretrained_state_dict = {k: v for k,
                                 v in pretrained_state_dict.items()}
    else:
        ...
        if torch_version == '1.3.0':
            pretrained_state_dict = {
                (
                    ...
                    k).replace(
                    '.denselayer',
                    '.layers.denselayer'): v for k,
                v in pretrained_state_dict.items() if 'classifier' not in k}
        else:
            pretrained_state_dict = {('Densenet.' + k): v for k,
                                     v in pretrained_state_dict.items() if 'classifier' not in k}
    # for key in pretrained_state_dict.keys():
    #     print(key, pretrained_state_dict[key].shape)
...
def sum_bbox_ratios(writer, vt_loss_allBoxes_list, tid, prefix, title):
    vt_loss_allBoxes_np = sorted(torch.stack(
        vt_loss_allBoxes_list).cpu().data.numpy().copy())
    ...
    thres_ratio_dict = summary_thres(
        vt_loss_allBoxes_np, [
            0.01, 0.02, 0.03, 0.05, 0.1, 0.2, 0.3, 0.5, 0.8, 1.], writer, 'thres_%s/%s' %
        (prefix, title), tid)
...
def print_f_info(f_estim, f_pixels_yannick_batch, input_dict):
    ...
    f_mm_list_yannick = []
    for f_single, H_single, W_single in zip(f_pixels_yannick_batch.cpu().detach(
    ).numpy(), input_dict['H_batch_array'], input_dict['W_batch_array']):
        f_single_mm = utils_coco.fpix_to_fmm(f_single, H_single, W_single)
        f_mm_list_yannick.append(f_single_mm)
    f_mm_list_est = []
    for f_single, H_single, W_single in zip(f_estim.cpu().detach(
    ).numpy(), input_dict['H_batch_array'], input_dict['W_batch_array']):
        f_single_mm = utils_coco.fpix_to_fmm(f_single, H_single, W_single)
...
def print_v0_info(v0_batch_est, v0_batch, output_pitch, H_batch):
    ...
    for idx, output_pitch_single in enumerate(output_pitch):
        v0_single_argmax_visible, v0_single_argmax = bin2midpointpitch(
            output_pitch_single.detach().squeeze().cpu().numpy())  # [top 0 bottom 1]
        if v0_single_argmax_visible:
            v0_argmax_image_single = H_batch[idx].cpu().numpy(
            ) - v0_single_argmax * H_batch[idx].cpu().numpy()  # [top H bottom 0];
        else:
