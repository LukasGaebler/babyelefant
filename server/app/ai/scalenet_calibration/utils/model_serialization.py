...
def align_and_update_state_dicts(
    ...
    if not (not replace_kws or not replace_with_kws):
        print('>> replace_kws:', replace_kws,
              '>> replace_with_kws:', replace_with_kws)
        assert len(replace_kws) == len(replace_with_kws) and len(
            replace_kws) > 0, 'Length of replace_kws %d and replace_with_kws %d should equal and > 0!' % (len(replace_kws), len(replace_with_kws))
        ...
        for current_key in current_keys:
            if_replace = False
            for replace_kw, replace_with_kw in zip(
                    replace_kws, replace_with_kws):
    ...
    # logger.warning('====== current_keys %d; loaded keys %d' %
        ...
        len(j) if i.replace(
            ...
            '').endswith(
                j.replace(
                    ...
                    '').replace(
                        '.layers',
                    '')) else 0 for i in current_keys_renamed for j in loaded_keys]
    ...
    # used for logging
    max_size = max([len(key) for key in current_keys_renamed]
                   ) if current_keys_renamed else 1
    ...
    # else:
        # logger.warning(red('====== Successfully loaded %d from %d possible loads.' % (
        #     success_loads, all_possible_loads)))
...
def load_state_dict(
    ...
    current_keys, loaded_keys = align_and_update_state_dicts(
        model_state_dict, loaded_state_dict, logger, skip_kws=skip_kws, only_load_kws=only_load_kws, replace_kws=replace_kws, replace_with_kws=replace_with_kws)
