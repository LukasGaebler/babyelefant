...
def show_cam_bbox(
    ...
    if 'yc_est_list' in input_dict:
        add_yc_list_str = '(%s)' % (', '.join(
            (['%.2f' % yc_est for yc_est in input_dict['yc_est_list']])))
        # print(input_dict['yc_est_list'], add_yc_list_str)
    else:
        add_yc_list_str = ''

    if 'f_pixels_est_mm_list' in input_dict:
        add_fmm_list_str = '(%s)' % (', '.join(
            (['%.2f' % fmm_est for fmm_est in input_dict['f_pixels_est_mm_list']])))
    else:
        add_fmm_list_str = ''

    if 'v0_est_list' in input_dict:
        add_v0_list_str = '(%s)' % (', '.join(
            (['%.2f' % v0_est for v0_est in input_dict['v0_est_list']])))
        add_v0_list_str = 'v0=%.2f' % sum(
            input_dict['v0_est_list']) + add_v0_list_str
        # print(input_dict['yc_est_list'], add_yc_list_str)
    else:
        add_v0_list_str = ''

    plt.title('[%s-%d-%s] H_camFit=%.2f, H_camEst=%.2f %s; f_est=%.2f mm %s; pitch=%.2f degree; %s' % (input_dict['task_name'].split('_')[0],
                                                                                                       input_dict['tid'],
    ...
    if 'v0_cocoPredict' in input_dict:
        ...
        plt.plot([0., W / 2.], [H - v0_cocoPredict, H -
                                v0_cocoPredict], 'w--', 'linewidth', 50)

    # SHOW HORIZON EST
    if 'v0_batch_predict' in input_dict:
        # (H = top of the image, 0 = bottom of the image)
        v0_batch_predict = input_dict['v0_batch_predict']
        plt.plot([0., W - 1.], [H - v0_batch_predict, H - \
                 v0_batch_predict], linestyle='-', linewidth=2, color='black')
    ...
    if 'v0_batch_est' in input_dict:
        # (H = top of the image, 0 = bottom of the image)
        v0_batch_est = input_dict['v0_batch_est']
        plt.plot([0., W - 1.], [H - v0_batch_est, H - v0_batch_est],
                 linestyle='-', linewidth=2, color='lime')
        if not if_not_detail:
            plt.text(
                W / 2.,
                H - v0_batch_est,
                'v0_est %.2f' % (1. - v0_batch_est / H),
                fontsize=8,
                weight="bold",
                color='lime',
                bbox=dict(
    ...
    if 'v0_batch_est_0' in input_dict:
        # (H = top of the image, 0 = bottom of the image)
        v0_batch_est = input_dict['v0_batch_est_0']
        plt.plot([0., W - 1.], [H - v0_batch_est, H - v0_batch_est],
                 linestyle='-', linewidth=2, color='aquamarine')
        if not if_not_detail:
            plt.text(
                0.,
                H -
    ...
    if 'bbox_h' in input_dict:
        if 'vt_camEst_N_delta_est_list' in input_dict:
            ...
            for sample_idx in range(len(input_dict['bbox_h'])):
                vt_camEst_N_delta_list_sample.append(
                    [vt_camEst_N_delta_layer[sample_idx] for vt_camEst_N_delta_layer in vt_camEst_N_delta_list])
            if not if_not_detail:
                for bbox, vt_camEst_person_delta_person_layers in zip(
                        input_dict['bbox_gt'], vt_camEst_N_delta_list_sample):
                    add_vt_camEst_person_delta_person_list_str = '(%s)' % (', '.join(
                        (['%.2f' % vt_camEst_person_delta_person for vt_camEst_person_delta_person in vt_camEst_person_delta_person_layers])))
                    plt.text(
                        bbox[0],
                        bbox[1] +
                        bbox[3],
                        'Err %s' %
                        (add_vt_camEst_person_delta_person_list_str),
                        fontsize=7,
                        bbox=dict(
        ...
        if 'person_hs_est_list' not in input_dict:
            for y_person, bbox in zip(
                    input_dict['bbox_h'], input_dict['bbox_gt']):
                plt.text(
                    bbox[0],
                    bbox[1],
                    '%.2f' %
            ...
            for sample_idx in range(len(input_dict['bbox_h'])):
                person_hs_est_list_sample.append(
                    [person_hs_est_layer[sample_idx] for person_hs_est_layer in person_hs_est_list])
            for y_person, bbox, y_person_layers in zip(
                    input_dict['bbox_h'], input_dict['bbox_gt'], person_hs_est_list_sample):
                if not if_not_detail:
                    add_y_person_list_str = '(%s)' % (', '.join(
                        (['%.2f' % y_person for y_person in y_person_layers])))
                else:
                    add_y_person_list_str = ''
                plt.text(
                    bbox[0], bbox[1], '%.2f %s' %
                    (y_person, add_y_person_list_str), fontsize=7, bbox=dict(
                        facecolor='white', alpha=0.5))
            if 'bbox_h_canonical' in input_dict:
                for y_person, y_person_canonical, bbox, y_person_layers in zip(
                        input_dict['bbox_h'], input_dict['bbox_h_canonical'], input_dict['bbox_gt'], person_hs_est_list_sample):
                    plt.text(
                        bbox[0], bbox[1] + 15, '%.2f C' %
                        (y_person_canonical), fontsize=10, bbox=dict(
                            facecolor='white', alpha=0.5))

    if 'bbox_loss' in input_dict and not if_not_detail:
        for vt_loss, bbox in zip(
                input_dict['bbox_loss'], input_dict['bbox_est']):
            plt.text(
                bbox[0] +
                bbox[2] -
                8,
...
def vis_output_softmax_argmax(output_camH, camH_bins, ax, title=''):
    ...
    plt.plot([camH_softmax_estim, camH_softmax_estim],
             [0, camH_max_prob * 1.05], '--')
    camH_argmax = camH_bins[np.argmax(output_camH)]
    plt.plot([camH_argmax, camH_argmax], [0, camH_max_prob * 1.05])
    plt.grid()
    ax.set_title('%s: softmax %.2f, argmax %.2f' %
                 (title, camH_softmax_estim, camH_argmax), fontsize='small')
...
def show_box_kps(
    ...
    image_sizes_ori = [
        (input_dict_show['W_batch_array'],
         input_dict_show['H_batch_array'])]
    ...
    for idx, (target, result) in enumerate(zip(target_list, result_list)):
        ...
        fig = plt.figure(figsize=(10 * figzoom, 10 * figzoom))
        plt.imshow(result)
        plt.title('[%d-%s]' % (input_dict_show['tid'],
                               input_dict_show['im_filename'][-6:]))
        # ax = plt.gca()
...
def vis_SUN360(
    ...
    if rank == 0 and if_vis:
        logger.info(
            green(
                'Visualizing SUN360..... potentially save to' +
                save_path))
    im2_list = []
    horizon_list = []
    pitch_list = []
    ...
    for idx in range(min(min_samples, len(im_paths))):
        ...
        if if_vis:
            ...
            im2, _ = showHorizonLine(im2, vfov, pitch, 0., focal_length=f_mm, color=(
                0, 0, 255), width=4)  # Blue: horizon converted from camera params WITHOUT roll
            im2 = showHorizonLineFromHorizon(im2, horizon, color=(
                255, 255, 0), width=4, debug=True)  # Yellow: est horizon v0
            ...
            im2, _ = showHorizonLine(
                im2, vfov, pitch, roll, focal_length=f_mm, debug=True, color=(
                    0, 0, 255), width=2)  # Blue: horizon converted from camera params with roll
            im2, _ = showHorizonLine(
                im2, vfov_gt, pitch_gt, roll_gt, focal_length=f_gt, debug=True, GT=True, color=(
                    255, 255, 255), width=1)  # White: GT horizon
...
#         ppitch = np.arctan((replace_v0_01 - 0.5) / 0.5 * np.tan(vvfov/2.))
    ...
    rendering_command = '%s %s --background --python %s' % (
        blender_path, scene_path, script_path)
...
#     plt.savefig(save_path + '/%s_noBbox.jpg'%(save_name), dpi = 100, bbox_inches='tight')
    ...
    v0_batch_from_pitch_vfov = input_dict_show['v0_batch_from_pitch_vfov']
    plt.plot([0., W], [H - v0_batch_from_pitch_vfov, H -
                       v0_batch_from_pitch_vfov], linestyle='-.', linewidth=5, color='blue')
    ...
    plt.text(
        ...
        100,
        r'$y_c$: %.2fm' %
        input_dict_show['yc_est'] +
        '\n' +
        r'$f_{mm}$: %.2fmm' %
        input_dict_show['f_est_mm'] +
        '\n' +
        r'$\theta$: %.2f$^\circ$' %
        input_dict_show['pitch_est_angle'],
        fontsize=50,
        color='white',
        bbox=dict(
            facecolor='black',
...
# fontsize=50, color='white', bbox=dict(facecolor='black', alpha=0.4))
